...
import { Check, X, Clock, Eye, AlertCircle, ShieldCheck, User, Users, FileText, ExternalLink, Globe } from 'lucide-react'
import api from '../../services/api'
import { useAuth } from '../../context/AuthContext'
import MediaFlowLogo from '../../components/common/MediaFlowLogo'

export default function EditorialDesk({ editionFilter }) {
  const navigate = useNavigate()
  const { user } = useAuth()
  const [articles, setArticles] = useState([])
  const [activeTab, setActiveTab] = useState('sub_editor')
  const [editorNotes, setEditorNotes] = useState({})
  const [processingId, setProcessingId] = useState(null)

  const isSubEditorOrAbove = ['sub_editor', 'editor', 'chief_editor'].includes(user?.role)
  const isSeniorEditorOrAbove = ['editor', 'chief_editor'].includes(user?.role)
  ...
  const handleSendToOnline = async (article) => {
    setProcessingId(article._id)
    try {
      await api.put(`/articles/${article._id}`, { edition: 'both' })
      await fetchArticles()
      alert('নিউজটি অনলাইন এডিশনে যুক্ত করা হয়েছে! এখন একজন অনলাইন এডিটরকে অ্যাসাইন করুন।')
      openAssignModal(article)
    } catch (err) {
      console.error('Failed to send to online', err)
      alert('অনলাইনে পাঠাতে সমস্যা হয়েছে')
    }
    setProcessingId(null)
  }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-8 bg-[#f8fafc] dark:bg-slate-900 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 dark:border-slate-800 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-black text-gray-900 dark:text-white mb-2 flex items-center gap-3">
            <span>📰</span> এডিটরিয়াল ডেস্ক ও মাল্টি-স্টেজ অনুমোদন
          </h1>
          <p className="text-sm text-gray-600 dark:text-slate-400 max-w-2xl">
            রিপোর্টার নিউজ জমা দেওয়ার পর তা ধাপে ধাপে যাচাই ও সম্পাদিত হয়: 
            <span className="font-semibold text-indigo-600 dark:text-indigo-400"> সাব-এডিটর → সিনিয়র এডিটর → চিফ এডিটর</span>।
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <button
          onClick={() => setActiveTab('sub_editor')}
          className={`p-4 rounded-xl border text-left transition-all ${
            activeTab === 'sub_editor'
        ...
        >
          <p className={`text-xs font-bold uppercase tracking-wider ${activeTab === 'sub_editor' ? 'text-indigo-100' : 'text-gray-400 dark:text-slate-500'}`}>
            ধাপ ১ (Stage 1)
          </p>
          <h3 className="text-sm font-bold mt-1">সাব-এডিটর কিউ</h3>
          <span className={`inline-block mt-2 px-2.5 py-0.5 text-xs font-bold rounded-full ${
            activeTab === 'sub_editor' ? 'bg-white text-indigo-600' : 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400'
          }`}>
            {queues.sub_editor.length}টি অপেক্ষায়
          </span>
        </button>
        ...
        >
          ...
          <h3 className="text-sm font-bold mt-1">সিনিয়র এডিটর কিউ</h3>
          <span className={`inline-block mt-2 px-2.5 py-0.5 text-xs font-bold rounded-full ${
            activeTab === 'senior_editor' ? 'bg-white text-indigo-600' : 'bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400'
          }`}>
            {queues.senior_editor.length}টি অপেক্ষায়
          </span>
        </button>

        <button
          onClick={() => setActiveTab('chief_editor')}
          className={`p-4 rounded-xl border text-left transition-all ${
            activeTab === 'chief_editor'
              ? 'bg-indigo-600 text-white border-indigo-600 shadow-md shadow-indigo-600/20'
              : 'bg-white dark:bg-slate-800 text-gray-700 dark:text-slate-300 border-gray-200 dark:border-slate-700 hover:border-indigo-400'
          }`}
        >
          <p className={`text-xs font-bold uppercase tracking-wider ${activeTab === 'chief_editor' ? 'text-indigo-100' : 'text-gray-400 dark:text-slate-500'}`}>
            ধাপ ৩ (Final)
          </p>
          <h3 className="text-sm font-bold mt-1">চিফ এডিটর কিউ</h3>
          <span className={`inline-block mt-2 px-2.5 py-0.5 text-xs font-bold rounded-full ${
            activeTab === 'chief_editor' ? 'bg-white text-indigo-600' : 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400'
          }`}>
            {queues.chief_editor.length}টি অপেক্ষায়
          </span>
        </button>
        ...
        >
          <p className={`text-xs font-bold uppercase tracking-wider ${activeTab === 'published' ? 'text-emerald-100' : 'text-gray-400 dark:text-slate-500'}`}>
            আর্কাইভ
          </p>
          <h3 className="text-sm font-bold mt-1">প্রকাশিত নিউজ</h3>
          <span className={`inline-block mt-2 px-2.5 py-0.5 text-xs font-bold rounded-full ${
            activeTab === 'published' ? 'bg-white text-emerald-600' : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300'
          }`}>
            {queues.published.length}টি
          </span>
        </button>
        ...
        <button
          ...
          className={`p-4 rounded-xl border text-left transition-all ${
            activeTab === 'rejected'
              ...
              : 'bg-white dark:bg-slate-800 text-gray-700 dark:text-slate-300 border-gray-200 dark:border-slate-700 hover:border-rose-400'
          }`}
        >
          <p className={`text-xs font-bold uppercase tracking-wider ${activeTab === 'rejected' ? 'text-rose-100' : 'text-gray-400 dark:text-slate-500'}`}>
            ফেরত পাঠানো
          </p>
          <h3 className="text-sm font-bold mt-1">সংশোধন প্রয়োজন</h3>
          <span className={`inline-block mt-2 px-2.5 py-0.5 text-xs font-bold rounded-full ${
            activeTab === 'rejected' ? 'bg-white text-rose-600' : 'bg-rose-50 dark:bg-rose-900/30 text-rose-600 dark:text-rose-400'
          }`}>
            {queues.rejected.length}টি
          </span>
        </button>
      </div>
                ...
                <div className="flex flex-col gap-2 pt-2">
                  ...
                  {a.edition === 'print' && (
                    <button
                      ...
                      onClick={() => handleSendToOnline(a)}
                      className="flex items-center justify-center gap-2 w-full py-2 px-4 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/50 font-bold text-xs rounded-xl transition-all"
                    >
                      <Globe size={14} />
                      <span>অনলাইন এডিশনে পাঠান (অ্যাসাইন করুন)</span>
                    </button>
                  )}

                  {activeTab === 'sub_editor' && isSubEditorOrAbove && (
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        disabled={processingId === a._id}
                        onClick={() => handleApprove(a._id, 'sub_editing')}
                        className="flex items-center justify-center gap-1 py-2 px-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl transition-all shadow-sm"
                      >
                        <Check size={14} />
                        <span>প্রুফ রিডারকে পাঠান</span>
                      </button>
                      <button
                        disabled={processingId === a._id}
                  ...
                  {activeTab === 'senior_editor' && isSeniorEditorOrAbove && (
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        disabled={processingId === a._id}
                        onClick={() => handleApprove(a._id, 'senior_review')}
                        className="flex items-center justify-center gap-1 py-2 px-3 bg-purple-600 hover:bg-purple-700 text-white font-bold text-xs rounded-xl transition-all shadow-sm"
                      >
                        <Check size={14} />
                        <span>চিফ এডিটরকে পাঠান</span>
                      </button>
                      
                      {editionFilter === 'online' ? (
                        <button
                          disabled={processingId === a._id}
                          onClick={() => handleApprove(a._id, 'chief_review')}
                          className="flex items-center justify-center gap-1 py-2 px-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl transition-all shadow-sm col-span-2 mt-1"
                        >
                          <Check size={14} />
                          <span>সরাসরি ওয়েবসাইটে পাবলিশ (Direct Publish)</span>
                        </button>
                      ) : (
                        <button
                          disabled={processingId === a._id}
                      ...
                      {editionFilter === 'online' && (
                        <button
                          disabled={processingId === a._id}
                          onClick={() => handleReject(a._id)}
                          className="flex items-center justify-center gap-1 bg-rose-50 hover:bg-rose-100 dark:bg-rose-900/20 text-rose-600 border border-rose-200 dark:border-rose-800/50 font-bold text-xs rounded-xl transition-all col-span-2"
                        >
                          <X size={14} />
                          <span>সংশোধনে ফেরত</span>
                        </button>
                  ...
                  {activeTab === 'chief_editor' && isChiefEditor && (
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        disabled={processingId === a._id}
                        onClick={() => handleApprove(a._id, 'chief_review')}
                        className="flex items-center justify-center gap-1 py-2 px-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl transition-all shadow-sm"
                      >
                        <Check size={14} />
                        <span>চূড়ান্ত প্রকাশ (Publish)</span>
                      </button>
                      <button
                        disabled={processingId === a._id}
                        onClick={() => handleReject(a._id)}
                  ...
                  {activeTab === 'rejected' && isSubEditorOrAbove && (
                    <button
                      disabled={processingId === a._id}
                      onClick={() => handleApprove(a._id, 'sub_editing')}
                      className="flex items-center justify-center gap-1 py-2 px-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-xl transition-all shadow-sm"
                    >
                      <Check size={14} />
                      <span>পুনরায় সাব-এডিটর কিউতে পাঠান</span>
                    </button>
                  )}
                </div>
              </div>
